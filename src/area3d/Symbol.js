...
import _labelHelper from 'echarts/lib/chart/helper/labelHelper';
import * as _symbol from './symbolUtil';
...
symbolProto._createSymbol = function(symbolType, data, idx, symbolSize) {
  ...
  var color =
    seriesModel.get('symbolLineColor') || data.getItemVisual(idx, 'color'); // var symbolPath = createSymbol(
...
symbolProto.link = function(data, idx, symbolType, symbolSize, seriesScope) {
  ...
  if (
    !symbolOffset ||
    Math.max(Math.abs(symbolOffset[0]), Math.abs(symbolOffset[1])) <
      symbolSize[0]
  ) {
    return;
  }
  ...
  var lineDash =
    symbolLineType === 'solid' || symbolLineType == null
      ? null
      : symbolLineType === 'dashed'
      ? [dashSize, dashSize]
      : [dotSize, dotSize];
...
symbolProto._updateCommon = function(data, idx, symbolSize, seriesScope) {
  ...
  if (!seriesScope || data.hasItemOption) {
    ...
    itemStyle = itemModel
      .getModel(normalStyleAccessPath)
      .getItemStyle(['color']);
