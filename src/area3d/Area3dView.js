...
zrUtil.extend(echarts.Model.prototype, Area3dStyle);
...
function createGridClipShape(cartesian, hasAnimation, seriesModel) {
  ...
  var expandSize = seriesModel.get('clipOverflow')
    ? lineWidth / 2
  ...
  return data.mapArray(
    ...
    function(val, idx) {
      ...
      stackedData[1 - baseDataOffset] = stackedOnSameSign
        ? stackedOnSameSign.get(valueDim, idx, true)
        : valueStart;
...
function createArea(
  ...
  createShape(
    ...
    RIGHT_FACE,
    {
      ...
      stackedOnPoints: stackedOnPoints[lastIndex]
        ? [stackedOnPoints[lastIndex], upStackedOnPoints[lastIndex]]
        : []
    },
    smooth,
    12,
  ...
  g.setClipPath(
    createGridClipShape(coordSys, !count ? hasAnimation : false, seriesModel)
  );
}
...
function setStyle(g, name, areaStyle, options) {
  g.childOfName(name).useStyle(zrUtil.defaults(options, areaStyle));
}
